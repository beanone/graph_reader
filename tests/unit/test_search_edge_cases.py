...
def test_search_expression_edge_cases():
    ...
    assert not evaluator.evaluate_condition(
        SearchCondition("tags", SearchOperator.EQUALS, "python"),
        {"tags": []}
    )

    # Test None value handling
    assert not evaluator.evaluate_condition(
        SearchCondition("name", SearchOperator.EQUALS, "test"),
        {"name": None}
    )
...
def test_search_expression_operators():
    ...
    assert not evaluator.evaluate_condition(
        SearchCondition("name", SearchOperator.GREATER_THAN, "test"),
        {"name": "test"}
    )

    # Test LESS_THAN with non-numeric values
    assert not evaluator.evaluate_condition(
        SearchCondition("name", SearchOperator.LESS_THAN, "test"),
        {"name": "test"}
    )

    # Test CONTAINS_TEXT with non-string values
    assert not evaluator.evaluate_condition(
        SearchCondition("count", SearchOperator.CONTAINS_TEXT, "test"),
        {"count": 123}
    )

    # Test STARTS_WITH with non-string values
    assert not evaluator.evaluate_condition(
        SearchCondition("count", SearchOperator.STARTS_WITH, "test"),
        {"count": 123}
    )

    # Test ENDS_WITH with non-string values
    assert not evaluator.evaluate_condition(
        SearchCondition("count", SearchOperator.ENDS_WITH, "test"),
        {"count": 123}
    )