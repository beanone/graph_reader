...
def test_web_search_scenarios():
    ...
    expr = parse_search_query('category:electronics AND price:<1000 AND (brand:apple OR brand:samsung)')
    assert isinstance(expr, SearchExpression)
    assert expr.operator == SearchOperator.AND

    # Content search with tags
    expr = parse_search_query('type:article AND (tags:python OR tags:programming) AND status:published')
    assert isinstance(expr, SearchExpression)
    assert expr.operator == SearchOperator.AND

    # Location-based search
    expr = parse_search_query('type:restaurant AND (cuisine:italian OR cuisine:japanese) AND rating:>4')
    assert isinstance(expr, SearchExpression)
    assert expr.operator == SearchOperator.AND

    # Job search
    expr = parse_search_query('type:job AND (title:~developer OR title:~engineer) AND location:~remote')
    assert isinstance(expr, SearchExpression)
    assert expr.operator == SearchOperator.AND
...
def test_pyparsing_basic():
    ...
    from pyparsing import Word, Literal, QuotedString, printables, ZeroOrMore
    ...
    value = Word(printables, excludeChars=":()/") + ZeroOrMore(Word(printables, excludeChars=":()/"))
    ...
    value = (Word(printables, excludeChars=":()/") + ZeroOrMore(Word(printables, excludeChars=":()/"))).setParseAction(lambda t: " ".join(t))
    result = value.parseString("john doe")
    print("\nTest 5 - Value with spaces using Word + ZeroOrMore with parse action:")
    print("Input: john doe")
    print("Result:", result.asList())
...
def test_pyparsing_array_values():
    ...
    print(f'Input: ["python", "java", "c++"]')
    print(f"Result: {result.asList()}")

    # Test 2: Array value with spaces
    print("\nTest 2: Array value with spaces")
    result = array_value.parseString('["python programming", "java development", "c++ coding"]')
    print(f'Input: ["python programming", "java development", "c++ coding"]')
    print(f"Result: {result.asList()}")

    # Test 3: Array value with mixed types
    print("\nTest 3: Array value with mixed types")
    result = array_value.parseString('["python", 123, "c++"]')
    print(f'Input: ["python", 123, "c++"]')
    print(f"Result: {result.asList()}")
    ...
    print(f'Input: tags:@["python", "java", "c++"]')
    print(f"Result: {result.asList()}")
    ...
    print(f'Input: tags:@["python", "java", "c++"]/i')
    print(f"Result: {result.asList()}")
    ...
    print("\nTest 9: Array with special characters")
    result = array_value.parseString('["user@domain.com", "file/path", "name-with-dashes"]')
    print(f'Input: ["user@domain.com", "file/path", "name-with-dashes"]')
    print(f"Result: {result.asList()}")
...
def test_pyparsing_logical_operators_handling():
    ...
    try:
        ...
        print(f"Input: name:john XOR age:25")
        print(f"Result: {result}")
    except ParseException as e:
        print(f"Expected error: {str(e)}")
...
def test_pyparsing_operator_precedence():
    ...
    print("\nTest 2: Complex nested conditions with multiple levels")
    result = expr.parseString("(name:john OR (name:jane AND age:25)) AND (city:london OR city:paris)")
    print(f"Input: (name:john OR (name:jane AND age:25)) AND (city:london OR city:paris)")
    print(f"Result: {result}")

    # Test 3: Multiple NOT operations
    print("\nTest 3: Multiple NOT operations")
    result = expr.parseString("NOT NOT name:john")
    print(f"Input: NOT NOT name:john")
    print(f"Result: {result}")
    ...
    print(f"Input: NOT (name:john AND (age:25 OR age:30))")
    print(f"Result: {result}")

    # Test 6: Deep nesting
    print("\nTest 6: Deep nesting")
    result = expr.parseString("((name:john OR name:jane) AND (age:25 OR age:30)) AND (city:london OR city:paris)")
    print(f"Input: ((name:john OR name:jane) AND (age:25 OR age:30)) AND (city:london OR city:paris)")
    print(f"Result: {result}")

    # Test 7: Mixed operators with parentheses
    print("\nTest 7: Mixed operators with parentheses")
    result = expr.parseString("(name:john AND age:25) OR (name:jane AND age:30)")
    print(f"Input: (name:john AND age:25) OR (name:jane AND age:30)")
    print(f"Result: {result}")
    ...
    try:
        ...
        print(f"Input: name:john AND OR age:25")
        print(f"Result: {result}")
    except ParseException as e:
        print(f"Expected error: {str(e)}")
    ...
    """)
    print(f"Input: Complex whitespace and formatting")
    print(f"Result: {result}")
